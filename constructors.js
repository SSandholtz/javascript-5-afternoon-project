...
function Car (make, model, year) {
  ...
  this.model = model
  this.year = year
  this.moveCar = function moveCar(value = 0) {
    value + 10
    return value
  }
}
...
Movie.prototype.changeRating = function changeRating(num) {
  let newRating = num
  let updatedRating = newRating + this.rating / 2
...
User.prototype.removeSavedPost = function (id) {
  let editedSavedPosts = this.savedPosts.filter(post => {
    return post.id !== id
  })
  this.savedPosts = editedSavedPosts
...
// Once you find the matching object, update it's rating score with the new rating parameter.	
 ...
 User.prototype.changePostRating = function (id, rating) {
    let postIndex = this.savedPosts.findIndex(post => {
      return post.id === id
    })
